...
const CREATE_PRESCRIPTION = gql`
  mutation createPrescriptionAdmin($input: CreatePrescriptionAdminInput!) {
    createPrescriptionAdmin(input: $input) {
      status
      message
    }
  }
...
const UPLOAD_FILE_URL = "https://api.mymultimeds.com/api/file/upload";
...
export default function Prescription() {
  ...
  const [createPrescriptionAdmin] = useMutation(CREATE_PRESCRIPTION, {
    onCompleted: (data) => {
      toast.success(data.createPrescriptionAdmin.message);
      refetch();
    },
    onError: (error) => {
      toast.error(`Error: ${error.message}`);
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-[16px]">
        ...
        <button
          className="text-white text-[16px] font-[500] leading-[20px] bg-[#7487FF] px-[16px] py-[8px] rounded-[4px]"
          onClick={addNewFile}
        >
          + Add New
        </button>
      ...
      {prescriptionList.map((item) => (
        <div
          className="flex justify-between items-center w-full py-[16px]"
          key={item.id}
        >
          <div className="flex justify-between bg-[#F8FAFC] w-[50%] p-[12px]">
            <p>{item.filename || "Prescription.pdf"}</p>
            <button
              className="text-[#7487FF] text-[12px] font-[500] leading-[15px]"
              onClick={() => handleDownload(item.url)}
            >
              View
            </button>
          </div>
          <p className="text-[#94A3B8] text-[12px] font-[400] leading-[15px] italic">
            Uploaded on{" "}
